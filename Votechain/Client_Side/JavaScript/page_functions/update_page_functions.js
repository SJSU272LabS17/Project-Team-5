...
function showEditTbl(el)
{
	...
	if($(el).siblings('.carVin').html() != '&lt;<i>VID</i>&gt;')
	{
...
function validate(el)
{
	...
	*/
	...
	{
		$('#errorRw').find('ul').append('<li>VID must be a number</li>')
		failed = true;
	}
	if($('#vin').val().trim().length != 15 && $('#vin').val().trim() != 0)
	{
		
		$('#errorRw').find('ul').append('<li>VID must be 15 characters (Currently ' + $('#vin').val().trim().length + ' characters)</li>')
		failed = true;
	}
	if($('#vin').val().trim() == 0 && $('#hidVin').val().trim() != 0)
	{
		$('#errorRw').find('ul').append('<li>VID cannot be reset to 0</li>')
		failed = true;
	}
	if($('#make').val().trim() == '')
	{
	...
	if($('#model').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Place cannot be blank</li>')
		failed = true;
	}
	if($('#model').val().trim().toLowerCase() == 'undefined' && $('#hidModel').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Place cannot be reset to undefined</li>')
		failed = true;
	}
	if($('#colour').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Party cannot be blank</li>')
		failed = true;
	}
	if($('#colour').val().trim().toLowerCase() == 'undefined' && $('#hidColour').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Party cannot be reset to undefined</li>')
		failed = true;
	}
	if($('#reg').val().trim() == '')
	{
